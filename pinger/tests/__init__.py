...
class PingerTests(CorptoolsTestCase):
    ...
    def setUp(cls):
        ...
        cls.eveName3 = EveName.objects.create(
            eve_id=3, name="Alli 3", category="alliance"
        )
        cls.eveName2 = EveName.objects.create(
            eve_id=2, name="Corp 2", alliance=cls.eveName3, category="corporation"
        )
        ...
        cls.typeName = EveItemType.objects.create(
            type_id=1, name="Item Type 1", published=True
        )
        cls.dateTime1Timestamp = 133772899408813831
        cls.dateTime1 = filetime_to_dt(cls.dateTime1Timestamp).replace(
        ...
        cls.region = MapRegion.objects.create(region_id=1, name="Region 1")
        cls.r1t = f"[Region 1](https://evemaps.dotlan.net/map/Region_1)"

        cls.constellation = MapConstellation.objects.create(
            constellation_id=1, name="Constellation 1", region=cls.region
        )
        ...
        cls.planet = MapSystemPlanet.objects.create(
            planet_id=1, name="Planet 1", x=0, y=0, z=0, system=cls.system
        )
