...
class DiscordWebhook(models.Model):
    ...
    corporation_filter = models.ManyToManyField(
        EveCorporationInfo, related_name="corp_filters", blank=True
    )

    alliance_filter = models.ManyToManyField(
        EveAllianceInfo, related_name="alli_filters", blank=True
    )

    region_filter = models.ManyToManyField(
        MapRegion, related_name="region_filters", blank=True
    )

    ping_types = models.ManyToManyField(PingType, blank=True)
...
class Ping(models.Model):
    ...
    def send_ping(self):
        ...
        tasks.send_ping.apply_async(priority=2, args=[self.id])
...
class FuelPingRecord(models.Model):
    ...
    )  # ozone remaining @last ping
    last_message = models.TextField(default="", blank=True)
    date_empty = models.DateTimeField(null=True, default=None, blank=True)  # expiry
    last_ping_time = models.IntegerField(
        null=True, default=None, blank=True
    )  # hours remaining @last ping
    ...
    def build_ping_ob(self, message):
        ...
        _services = ",".join(
            self.structure.structureservice_set.filter(state="online").values_list(
                "name", flat=True
            )
        )
        if len(_services) == 0:
        ...
        custom_data = {
            ...
            "description": message,
            "fields": [
                {"name": "System", "value": _system_name, "inline": True},
                {"name": "Region", "value": _region_name, "inline": True},
            ],
        }
        ...
        if self.structure.fuel_expires:
            ...
            custom_data["fields"].append(
                {"name": "Days Remaining", "value": str(daysLeft), "inline": True}
            )
            custom_data["fields"].append(
    ...
    def ping_task_ob(self, message):
        ...
        webhooks = DiscordWebhook.objects.filter(fuel_pings=True).prefetch_related(
...
class PingerConfig(models.Model):
    ...
    AllianceLimiter = models.ManyToManyField(
        EveAllianceInfo, blank=True, help_text="Alliances to put into the queue"
    )
    CorporationLimiter = models.ManyToManyField(
        EveCorporationInfo, blank=True, help_text="Corporations to put into the queue"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.pk = self.id = (
            1  # If this happens to be deleted and recreated, force it to be 1
        )
